...
def get_mock_data():
    ...
    columns = ['콜사인(이름)', '국가', 'lon', 'lat', '기압고도(m)', '속도(m/s)', '방향(각도)']
...
def get_real_flights_opensky(username="", password=""):
    ...
    try:
        ...
        flight_list = []
        for s in states:
            # OpenSky API 문서의 인덱스 매핑 가이드:
            # s[1]: 콜사인 (문자열 끝에 공백이 있을 수 있어 strip() 처리)
            # s[2]: 출발 국가
            # s[5]: 경도(longitude)
            # s[6]: 위도(latitude)
            # s[7]: 기압 고도(barometric altitude, 미터 단위) - None일 수 있음
            # s[9]: 속도(ground speed, m/s 단위) - None일 수 있음
            # s[10]: 방향각(true track, 북쪽 기준 시계방향 각도) - None일 수 있음
...
with st.sidebar:
    ...
    st.write("아이디 없이도 호출이 가능하지만, 하루 요청 수가 매우 제한되어 있습니다. [OpenSky Network](https://opensky-network.org/)에 무료 회원가입 후 아래에 입력하면 안정적으로 작동해요!")
...
if not flights_df.empty:
    ...
    column_layer = pdk.Layer(
        ...
        get_elevation="기압고도(m)",
        elevation_scale=1.5,       # 고도 표현의 입체감을 높이기 위해 스케일 조절
        radius=3000,               # 기둥 두께 설정
        get_fill_color=["50", "150 + (기압고도(m) / 100)", "255", "200"], # 높을수록 밝은 파란색 계열
        pickable=True,
        auto_highlight=True,
    )
    ...
    tooltip = {
        "html": "<b>콜사인:</b> {콜사인(이름)} <br/>"
                "<b>출발국가:</b> {국가} <br/>"
                "<b>실시간 고도:</b> {기압고도(m)} m <br/>"
                "<b>지속도:</b> {속도(m/s)} m/s (약 {속도(m/s)*3.6:.1f} km/h)",
